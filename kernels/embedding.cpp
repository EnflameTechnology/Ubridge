...
__device__ void rope(T* query, T* key, CST* cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
    ...
    __local__ int64_t positions[16 * 1024];

    tops::memcpy(ctx, tops::mdspan(tops::Private, positions, num_tokens), 
            tops::mdspan(tops::Global, index_positions, num_tokens));
    ...
    for (int p = 0; p < thread_step; p++) {
      int64_t idx = thread_id * THREAD_STEP + p;
      if (idx >= N || positions[idx] >= cos_sin_length) break;
      // int batch_idx = idx / num_tokens;
      CST* cos_sin_cur = cos_sin + positions[idx] * (int64_t)cos_sin_stride;
      // int64_t i = idx;// % num_tokens;
      auto query_sub0 = query + idx * q_stride;
      auto key_sub0 = key + idx * k_stride;
...
extern "C" __global__ void  rope_f32(float *query, float *key, float *cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
                      int num_tokens, int q_heads, int k_heads, int hidden_size, int split_dim, int gpt_geox) {
      rope<float, float>(
        query, key, cos_sin, cos_sin_length, cos_sin_stride, index_positions,
        num_tokens, q_heads, k_heads, hidden_size, split_dim, gpt_geox);
}

extern "C" __global__ void  rope_f16(__fp16*query, __fp16 *key, __fp16 *cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
                      int num_tokens, int q_heads, int k_heads, int hidden_size, int split_dim, int gpt_geox) {
    rope<__fp16, __fp16>(
      query, key, cos_sin, cos_sin_length, cos_sin_stride, index_positions,
      num_tokens, q_heads, k_heads, hidden_size, split_dim, gpt_geox);
...
extern "C" __global__ void  rope_bf16(__bf16 *query, __bf16 *key, __bf16 *cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
                       int num_tokens, int q_heads, int k_heads, int hidden_size, int split_dim, int gpt_geox) {
    rope<__bf16, __bf16>(
      query, key, cos_sin, cos_sin_length, cos_sin_stride, index_positions,
      num_tokens, q_heads, k_heads, hidden_size, split_dim, gpt_geox);
...
extern "C" __global__ void  rope_f32_bf16(__bf16 *query, __bf16 *key, float *cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
                       int num_tokens, int q_heads, int k_heads, int hidden_size, int split_dim, int gpt_geox) {
    rope<__bf16, float>(
      query, key, cos_sin, cos_sin_length, cos_sin_stride, index_positions,
      num_tokens, q_heads, k_heads, hidden_size, split_dim, gpt_geox);
...
extern "C" __global__ void  rope_f32_f16(__fp16 *query, __fp16 *key, float *cos_sin, int cos_sin_length, int cos_sin_stride, int64_t* index_positions,
                       int num_tokens, int q_heads, int k_heads, int hidden_size, int split_dim, int gpt_geox) {
    rope<__fp16, float>(
      query, key, cos_sin, cos_sin_length, cos_sin_stride, index_positions,
      num_tokens, q_heads, k_heads, hidden_size, split_dim, gpt_geox);
